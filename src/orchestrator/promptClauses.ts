...
export const ACCEPTANCE_SCOPE_CLAUSE = `=== 验收范围（硬约束）===
- 验收范围比仓库内可见样本更广：最终验收会在你从未见过的输入形态上检查同一修复目标；可见样本通过不构成验收通过。`;

export const CONTRACT_COMPLETENESS_CLAUSE = `=== 契约完整性（硬约束）===
...
export const PROBER_GUIDANCE_STATUS_CLAUSE = `=== 来自 prober 的一切内容都是「可错的一家之言」（★ 认真读，但不许奉为金科玉律 ★）===
...
export const KILL_LINE_CLAUSE = `=== 假设 kill line（prober 的一家之言：导航线索，★ 不是验收门禁 ★）===
...
export const SELF_STRESS_CHECK_CLAUSE = `提交前自检：枚举模块 I/O 契约的全部输入维度，用自生成的压力输入在本地逐维度验证修复。`;

export const MENTOR_RUBRIC_CLAUSE = `=== scientific-aware rubric（硬约束）===
...
export const HIDDEN_CONTRACT_QUOTA_CLAUSE = `=== 隐藏契约交付（★ 这是你的第一交付物，硬约束 ★）===
...
export const PROBE_AS_EVIDENCE_CLAUSE = `=== 探针（可选辅助证据，★ 不再是第一交付物 ★）===

...
exports.expertRoles = expertRoles;
exports.runExperts = runExperts;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const runner_1 = require("./runner");
const schemas_1 = require("./schemas");
const promptClauses_1 = require("../orchestrator/promptClauses");
...
function expertRoles() {
    return EXPERT_ROLES.map((e) => e.role);
}
function buildExpertPrompt(role, brief, intake, knowledge, knowledgeDir, timeLimitMs, probeSkill) {
    const probeSkillSection = probeSkill?.trim()
...
${promptClauses_1.HIDDEN_CONTRACT_QUOTA_CLAUSE}

${promptClauses_1.PROBE_AS_EVIDENCE_CLAUSE}
...
function recoverFromJsonl(jsonlPath) {
    try {
        ...
        if (!lastText)
            return undefined;
        return (0, schemas_1.extractJson)(lastText);
    }
    catch {
        return undefined;
    }
...
/** 并行运行全部专家；单个失败不阻塞其他。 */
async function runExperts(intake, options, modelOverrides, keyOverrides, providerOverrides, knowledgeOverrides, knowledgeDir, onProgress, expertRoles, probeSkill, 
/**
 * ★ 本底判别诊断（确定性，非 LLM；只标注信息量，不判分、不剔除）★：在**未修复的工作区**上真跑一遍该角色的探针。
 * 返回 executed=false 表示诊断自身没跑起来（环境问题），此时不做判定、不阻塞出题。
 */
probeBaseline) {
    const expertOptions = { ...options, sandbox: 'danger-full-access' };
    const active = expertRoles
        ? EXPERT_ROLES.filter((e) => expertRoles.includes(e.role))
        : EXPERT_ROLES;
    const runOne = async (expert) => {
        onProgress?.(expert.role, 'started');
        const prompt = buildExpertPrompt(expert.role, expert.brief, intake, knowledgeOverrides?.[expert.role], knowledgeDir, options.timeoutMs, probeSkill);
        const roleOptions = {
            ...expertOptions,
        ...
        let result = await (0, runner_1.runCodexOnce)(prompt, roleOptions);
        let report = result.ok ? (0, schemas_1.extractJson)(result.text) : undefined;
        // stdout 解析/schema 失败时，从 claude 会话 jsonl（raw/<role>.jsonl）兜底恢复最终报告
        if (result.ok && !(0, schemas_1.validateProbeReport)(report) && options.rawJsonlDir) {
            const recovered = recoverFromJsonl(path.join(options.rawJsonlDir, `${expert.role}.jsonl`));
            if (recovered && (0, schemas_1.validateProbeReport)(recovered)) {
                report = recovered;
                onProgress?.(expert.role, 'recovered-from-jsonl');
            }
        }
        // schema 不合格则补正一次（resume 同一会话）
        if (result.ok && !(0, schemas_1.validateProbeReport)(report) && result.threadId) {
            result = await (0, runner_1.runCodexOnce)('你的输出不符合 schema。请只输出修正后的 JSON 对象，字段完整，不要输出其他内容。', { ...roleOptions, resumeThreadId: result.threadId });
            report = result.ok ? (0, schemas_1.extractJson)(result.text) : undefined;
        }
        // ★ 隐藏契约交付确定性检查（非 LLM；★ 非阻塞 ★）：未达标 → 补正一次；仍缺 → 放行并记录告警
        let coverageMissing;
        let contractCov;
        if (result.ok && (0, schemas_1.validateProbeReport)(report)) {
            const coverage = (0, schemas_1.contractCoverage)(report);
            contractCov = coverage;
            coverageMissing = coverage.legacy ? [] : coverage.missing;
            if (coverageMissing.length > 0 && result.threadId) {
                result = await (0, runner_1.runCodexOnce)(`你的报告隐藏契约交付未达标，缺失项：${coverageMissing.join('、')}。` +
                    `请补齐：hidden_contracts 至少 ${schemas_1.CONTRACT_QUOTAS.minContracts} 条、至少 ${schemas_1.CONTRACT_QUOTAS.minKinds} 种不同 kind、` +
                    `structure_map 至少 ${schemas_1.CONTRACT_QUOTAS.minChains} 条入口链；每条契约必须含 ` +
                    `statement / kind / locus / discovered_via / evidence(可复核) / violates_if / confidence / open_question / patcher_action。` +
                    '只输出修正后的完整 JSON 对象，不要输出其他内容。', { ...roleOptions, resumeThreadId: result.threadId });
                const rerun = result.ok ? (0, schemas_1.extractJson)(result.text) : undefined;
                if (rerun && (0, schemas_1.validateProbeReport)(rerun)) {
                    report = rerun;
                    const c2 = (0, schemas_1.contractCoverage)(rerun);
                    contractCov = c2;
        ...
        let baselineGate;
        if (result.ok && (0, schemas_1.validateProbeReport)(report) && probeBaseline) {
            try {
            ...
            if (baselineGate?.executed && lowSignalProbes.length > 0 && result.threadId) {
                ...
                result = await (0, runner_1.runCodexOnce)(`${baselineGate.feedback}\n\n` +
                    ...
                    '只输出该 JSON，不要输出任何解释。', { ...roleOptions, resumeThreadId: result.threadId });
                const rerun = result.ok ? (0, schemas_1.extractJson)(result.text) : undefined;
                if (rerun && (0, schemas_1.validateProbeReport)(rerun)) {
                    report = rerun;
                    const c3 = (0, schemas_1.contractCoverage)(rerun);
                    contractCov = c3;

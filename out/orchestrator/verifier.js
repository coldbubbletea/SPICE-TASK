...
exports.materializeProbes = materializeProbes;
exports.loadProbesFromRunDir = loadProbesFromRunDir;
exports.verify = verify;
exports.runProbeBaselineGate = runProbeBaselineGate;
exports.aggregateProbeBaselineGates = aggregateProbeBaselineGates;
...
function resolvePython(workspaceRoot, configured) {
    if (configured.trim()) {
...
function runCommand(command, args, cwd, env) {
    return new Promise((resolve) => {
        (0, child_process_1.execFile)(command, args, { cwd, timeout: PROBE_TIMEOUT_MS, maxBuffer: 4 * 1024 * 1024, env }, (err, stdout, stderr) => {
...
async function runProbe(probe, probesDir, workDir, pythonPath, env) {
    ...
    const result = probe.kind === 'python'
...
function materializeProbes(runDir, probes, relDir = 'probes') {
    if (probes.length === 0)
        return undefined;
 ...
 */
function loadProbesFromRunDir(runDir) {
    const dir = path.join(runDir, 'probes');
    const manifestFile = path.join(dir, 'manifest.json');
    if (fs.existsSync(manifestFile)) {
        try {
            const manifest = JSON.parse(fs.readFileSync(manifestFile, 'utf8'));
            const list = Array.isArray(manifest) ? manifest : manifest.probes ?? [];
            return list
                .filter((m) => !!m && typeof m.file === 'string')
                .map((entry) => ({
                id: String(entry.id ?? ''),
                title: String(entry.title ?? ''),
                kind: entry.kind === 'shell' ? 'shell' : 'python',
    ...
    return fs
        ...
        id: f.replace(/\.(py|sh)$/, ''),
        title: f,
        kind: (f.endsWith('.py') ? 'python' : 'shell'),
 ...
 */
async function verify(workDir, probes, publicCommand, runDir, pythonPath) {
...
const ENV_UNAVAILABLE_PATTERNS = [/No module named '(numpy|pydicom)['"]?/, /No module named/];
/**
 ...
 */
const PUBLIC_ENV_FAILURE_PATTERNS = [
    /ModuleNotFoundError/,
    /No module named/,
    /ImportError/,
    /command not found/,
 ...
 * 仅用于证据与反射提示，不参与剔除判定。
 */
function attributeFailure(probeId, output) {
...
function classifyBaselineResult(probe, exitCode, output) {
    ...
    const classification = exitCode === 0 ? 'non_discriminating' : broken ? 'broken' : 'discriminating';
 ...
 */
async function runProbeBaselineGate(workspaceRoot, probes, runDir, pythonPath) {
    const empty = {
        ok: false,
        executed: false,
        reason: 'no probes',
    ...
    const results = baseline.probe_results.map((r) => {
        const probe = byId.get(r.probe_id) ?? { id: r.probe_id, title: r.title };
        const cls = classifyBaselineResult(probe, r.passed ? 0 : 1, r.output);
        const attr = r.passed ? { attribution: 'self' } : attributeFailure(r.probe_id, r.output);
        return {
            probe_id: r.probe_id,
            title: r.title,
    ...
    const envUnavailable = broken.length === results.length &&
        broken.length > 0 &&
    ...
    const lowSignal = results.filter((r) => r.classification === 'non_discriminating' || r.classification === 'broken');
    ...
    if (discriminating.length > 0)
        lines.push(`有判别力：${discriminating.join(', ')}`);
    if (falseGreenBlocking.length > 0) {
        lines.push(`★ 假绿（在本底上就通过 = 对本次修复零判别力）：${falseGreenBlocking.map((r) => `${r.probe_id}[${r.priority}]`).join(', ')}`);
    }
    if (broken.length > 0)
        lines.push(`★ 坏探针（自身跑不起来）：${broken.map((r) => `${r.probe_id}[${r.priority}]`).join(', ')}`);
    if (discriminating.length === 0)
        lines.push('★ 没有任何探针在本底上失败：套件对本 bug 完全无判别力。');
    const feedbackLines = [
    ...
    for (const r of results) {
        if (r.classification === 'discriminating') {
            ...
            feedbackLines.push(`- ${r.probe_id} [${r.priority}] ★ 本底**通过** → 假绿。它在修复前后行为相同，无法验收任何东西。` +
                `若你要保留它的判别力，可改断言契约要求中「本底尚未满足」的可观测量（probe_skill.md §7.3 本底协议 / §7.2 探针字段）；` +
                `★ 不是门禁：也可以原样保留并标 low_signal —— 本底通过的探针照交不误（§7.3）。`);
        }
        else {
            feedbackLines.push(`- ${r.probe_id} [${r.priority}] ★ 探针自身跑不起来 → 坏探针。请修正其导入/语法/路径（见下方实测输出），` +
                `不要把它当成「已发现 bug」。`);
        }
        ...
        if (r.classification === 'discriminating' && r.attribution === 'external') {
            feedbackLines.push(`  ★ 归因存疑（probe_skill.md §7.3 失败归因）：最深 traceback 帧在 ${r.attributed_frame ?? '被测代码内'}，` +
            ...
            feedbackLines.push('  ★ 归因不明（probe_skill.md §7.3 失败归因）：输出里没有可解析的 Python traceback 帧（超时 / 非 Python 运行器）。' +
                '请在报告里给出归因证据（探针自身断言处的 file:line 与实测值），否则该探针不算判别。');
        }
    }
    feedbackLines.push('');
    feedbackLines.push('说明（★ 仅供参考，不是门禁 ★）：以上是探针在本底上的实测信号，用于帮助你判断自己的探针是否有信息量。' +
 ...
 */
function aggregateProbeBaselineGates(gates) {
    ...
    for (const gate of usable) {
        for (const r of gate.results) {
            if (!byId.has(r.probe_id))
                byId.set(r.probe_id, r);
        }
    }
    ...
    if (discriminating.length === 0)
        lines.push('★ 没有任何探针在本底上失败：套件对本 bug 无判别力。');

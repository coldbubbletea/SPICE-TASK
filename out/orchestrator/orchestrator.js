...
exports.findShotArtifacts = findShotArtifacts;
exports.findRoleSkillFile = findRoleSkillFile;
exports.findProbeSkill = findProbeSkill;
exports.findPatcherSkill = findPatcherSkill;
exports.findReviewerSkill = findReviewerSkill;
...
const promptClauses_1 = require("./promptClauses");
...
const PROBER_AGENT_NAMES = { tester: 'ds', kimi: 'kimi' };
const PATCHER_AGENT_NAMES = { sci: 'qwen', general: 'glm', ds: 'ds' };
/** 写入 agent 自名声明文件 <runDir>/<kind>/<selfName>/<selfName>.md（角色/模型/提供商/时间戳/计划产物清单）。 */
function writeAgentSelfDecl(runDir, kind, selfName, fields) {
    const lines = [
        `# ${kind}/${selfName} — 自名声明`,
        '',
    ...
    (0, schemas_1.writeArtifact)(runDir, `${kind}/${selfName}/${selfName}.md`, lines.join('\n'));
}
/** 活动日志节流器：动作实时收集，每 3 分钟汇总一行落盘并上屏。 */
function makeActivityLogger(runDir, emit) {
...
function mergeReports(reports) {
    ...
    const seenChainEntries = new Set();
 ...
 */
function materializeContracts(dir, contracts, chains, relDir = 'verify') {
    if (contracts.length === 0 && chains.length === 0)
        return undefined;
    ...
    if (contracts.length > 0) {
        ...
        for (const c of contracts) {
            ...
            if (c.confidence !== undefined && c.confidence !== '')
                lines.push(`- prober 自评置信度: ${c.confidence}`);
            if (c.scope)
                lines.push(`- 适用范围: ${c.scope}`);
            lines.push(`- 违反即（反例风险）: ${c.violates_if}`);
            lines.push(`- 要求你要做的: ${c.patcher_action}`);
            if (c.probe_ids?.length)
                lines.push(`- 相关探针（可选证据）: ${c.probe_ids.join(', ')}`);
    ...
    if (chains.length > 0) {
        ...
        for (const ch of chains) {
            ...
            if (ch.siblings?.length)
                lines.push(`  - 兄弟调用点（共享同一契约，改一处要一并评估）: ${ch.siblings.join(' | ')}`);
            if (ch.note)
                lines.push(`  - 备注: ${ch.note}`);
        }
        lines.push('');
    }
    fs.writeFileSync(path.join(outDir, 'contracts.md'), lines.join('\n'));
    fs.writeFileSync(path.join(outDir, 'contracts.json'), JSON.stringify({ source: 'probe-stage', hidden_contracts: contracts, structure_map: chains }, null, 2));
    return path.join(outDir, 'contracts.md');
}
function buildRepairPrompt(intake, reports, retryFeedback, framing, timeLimitMs, baseline) {
...
${merged.contracts
            .map((c) => {
            const conf = c.confidence === undefined || c.confidence === '' ? '' : `（prober 自评置信度 ${c.confidence}）`;
            const lines = [
            ...
            if (c.probe_ids?.length)
                lines.push(`  相关探针: ${c.probe_ids.join(', ')}（可选证据，见下文探针段）`);
            return lines.join('\n');
        })
            .join('\n')}
...
`
    ...
    const lowSignalLines = (baseline?.results ?? [])
        ...
        .map((r) => {
        const why = r.classification === 'non_discriminating'
    ...
    const killLineBlock = killLineProbes.length
        ? `=== ★ 导航线索：假设 kill line（建议逐个亲自运行；先用未修改代码确认本底结论，改完再跑同一条看是否翻转）★ ===\n${killLineProbes
            ...
            const alt = typeof p.alt_sites === 'string' && p.alt_sites.trim()
    ...
    const timeBlock = typeof timeLimitMs === 'number' && timeLimitMs <= 0
...
${promptClauses_1.ACCEPTANCE_SCOPE_CLAUSE}

${promptClauses_1.CONTRACT_COMPLETENESS_CLAUSE}

${promptClauses_1.KILL_LINE_CLAUSE}

${promptClauses_1.PROBER_GUIDANCE_STATUS_CLAUSE}
...
function buildDualRetryFeedback(semanticIssues, guidance, comparison) {
    const lines = ['上一轮三位 patcher 的补丁均未通过 SciReviewer 终审。'];
...
function buildPrevRoundPatcherFeedback(runDir) {
    let latestRound = 0;
    ...
    const comparison = JSON.parse(fs.readFileSync(path.join(runDir, `comparison-r${round}.json`), 'utf8'));
    const lines = [
    ...
    for (const channel of ['sci', 'general', 'ds']) {
...
async function capturePatch(workspaceRoot, runDir, name) {
    const content = await new Promise((resolve) => {
        (0, child_process_1.execFile)('git', ['-C', workspaceRoot, 'diff'], { maxBuffer: 32 * 1024 * 1024 }, (err, stdout, stderr) => {
            resolve(err
                ? `# 无法获取 git diff（工作区可能不是 git 仓库）：${stderr || err}`
                : (stdout.trim() || '# 本轮 repair 未产生代码改动') + '\n');
        });
    });
    (0, schemas_1.writeArtifact)(runDir, `patch-${name}.diff`, content);
    return content;
}
/** 把工作区主目录复制到临时工作副本（排除 .codex-sci-debug；dest 可能嵌套在 source 内，必须用 tar 排除法）。 */
function makeWorkCopy(sourceRoot, dest) {
...
async function applyPatchToWorkspace(workspaceRoot, patchText, runDir, label) {
    ...
    try {
        (0, child_process_1.execFileSync)('git', ['-C', workspaceRoot, 'checkout', '--', '.'], { stdio: 'pipe' });
    }
    catch {
        // 非 git 仓库：跳过还原
    }
    ...
    for (const args of argsList) {
        const ok = await new Promise((resolve) => {
            (0, child_process_1.execFile)('git', args, { maxBuffer: 16 * 1024 * 1024 }, (err) => resolve(!err));
        });
        if (ok) {
            return true;
        }
...
async function patchAppliesToWorkspace(workspaceRoot, patchText, name) {
    const file = path.join(os.tmpdir(), `sci-debug-patchcheck-${name}.diff`);
    fs.writeFileSync(file, patchText + (patchText.endsWith('\n') ? '' : '\n'));
    try {
        return await new Promise((resolve) => {
            (0, child_process_1.execFile)('git', ['-C', workspaceRoot, 'apply', '--check', '-p1', '--whitespace=nowarn', file], { maxBuffer: 16 * 1024 * 1024 }, (err) => resolve(!err));
        });
    }
    finally {
...
function findShotArtifacts(startDir) {
    const root = path.resolve(startDir);
    const taskId = path.basename(root);
    const extFilter = (name) => /[.](diff|patch|md|txt)$/i.test(name);
    let scopedShot;
    ...
    for (let depth = 0; depth < 12; depth++) {
        ...
        if (genericShot === undefined) {
            ...
            if (fs.existsSync(shotCandidate))
                genericShot = shotCandidate;
        }
...
function findRoleSkillFile(startDir, fileName) {
    const root = path.resolve(startDir);
    const taskId = path.basename(root);
    let taskScoped;
    const shared = [];
        ...
        if (fs.existsSync(shotsDir) && fs.statSync(shotsDir).isDirectory()) {
            for (const entry of fs.readdirSync(shotsDir, { withFileTypes: true })) {
                ...
                if (!fs.existsSync(skillPath))
                    continue;
                if (entry.name === taskId && taskScoped === undefined)
                    taskScoped = skillPath;
                else
                    shared.push(skillPath);
            }
            const genericInShots = path.join(shotsDir, fileName);
            if (fs.existsSync(genericInShots))
                shared.push(genericInShots);
        }
...
function findProbeSkill(startDir) {
    return findRoleSkillFile(startDir, 'probe_skill.md');
}
function findPatcherSkill(startDir) {
    return findRoleSkillFile(startDir, 'patcher_skill.md');
}
function findReviewerSkill(startDir) {
...
function pickByScore(c) {
    const score = (o) => {
        return o.verify.public_ok === true ? 1 : 0;
    };
    const candidates = [c.sci, c.general, c.ds];
...
async function rebuildComparisonFromPrevious(prevDir, runDir, options, probes) {
    const buildOne = async (source) => {
        ...
        if (fs.existsSync(prevVerifyFile)) {
            (0, schemas_1.writeArtifact)(runDir, `verify-previous-r1-${source}.json`, JSON.parse(fs.readFileSync(prevVerifyFile, 'utf8')));
        }
        (0, schemas_1.writeArtifact)(runDir, `patch-r1-${source}.diff`, patchText || '# （断点复用）该候选未交出任何 diff');
        ...
        const verifyResult = await (0, verifier_1.verify)(copy, probes, options.publicCommand, runDir, options.pythonPath);
        (0, schemas_1.writeArtifact)(runDir, `verify-result-r1-${source}.json`, verifyResult);
        const checkOk = applied && (await patchAppliesToWorkspace(options.workspaceRoot, patchText, `r1-${source}-resume`));
        if (!checkOk) {
            (0, schemas_1.writeArtifact)(runDir, `verify-result-r1-${source}.json`, {
                ...verifyResult,
                ok: false,
...
async function runPipeline(intake, options, emit) {
    ...
    if (reuseDir && fs.existsSync(prevStateFile)) {
        state = JSON.parse(fs.readFileSync(prevStateFile, 'utf8'));
        state.stage = roleMode === 'patcher' ? 'repair' : roleMode === 'mentor' ? 'mentor' : state.stage;
    }
    else {
        state = { runId, stage: 'intake', retries: 0, failedExperts: [] };
    }
    (0, schemas_1.writeArtifact)(runDir, 'intake.md', intake);
    (0, schemas_1.writeArtifact)(runDir, 'run-state.json', state);
    ...
    if (probeSourceDir) {
        for (const role of options.expertRoles ?? ['tester', 'kimi']) {
            ...
            if (fs.existsSync(file)) {
                ...
                (0, schemas_1.writeArtifact)(runDir, `probe-report-${role}.json`, report);
                (0, schemas_1.writeArtifact)(runDir, `prober/${agentName}/probe-report-${agentName}.json`, report);
                (0, verifier_1.materializeProbes)(runDir, report.probes, `prober/${agentName}/tests`);
        ...
        if (reports.length === 0) {
            const loaded = (0, verifier_1.loadProbesFromRunDir)(probeSourceDir);
            if (loaded.length > 0) {
                const loadedReport = {
                    role: 'resume',
                    questions: [],
                ...
                (0, schemas_1.writeArtifact)(runDir, 'probe-report-resume.json', loadedReport);
        ...
        const probeBaseline = options.probeBaselineGate === false
            ? undefined
            : async (role, probes) => {
                if (probes.length === 0) {
                    return {
                ...
                const gate = await (0, verifier_1.runProbeBaselineGate)(options.workspaceRoot, probes, gateDir, options.pythonPath);
                (0, schemas_1.writeArtifact)(runDir, `baseline-gate/probe-baseline-gate-${role}.json`, gate);
                activity.record(`[baseline] ${role}: ${gate.summary}`);
                emit({ type: 'activity', text: `🔬 [baseline] 本底判别诊断 ${role}: ${gate.summary}` });
                return gate;
            };
        const expertResults = await (0, experts_1.runExperts)(intake, { ...optionsWithActivity, timeoutMs: options.expertTimeoutMs ?? 20 * 60 * 1000, rawLogDir: path.join(runDir, 'raw'), rawJsonlDir: path.join(runDir, 'raw') }, options.expertModels, options.expertKeys, options.expertProviders, options.expertKnowledge, options.knowledgeDir, (role, status) => emit({ type: 'expert', role, status }), options.expertRoles, probeSkill, probeBaseline);
        for (const result of expertResults) {
            if (result.baseline_gate)
                baselineGates.push(result.baseline_gate);
            if (result.ok && result.report) {
                ...
                (0, schemas_1.writeArtifact)(runDir, `probe-report-${result.role}.json`, result.report);
                (0, schemas_1.writeArtifact)(runDir, `prober/${agentName}/probe-report-${agentName}.json`, result.report);
                (0, verifier_1.materializeProbes)(runDir, result.report.probes, `prober/${agentName}/tests`);
    ...
    const coverageAudit = reports.map((rep) => {
        const c = (0, schemas_1.contractCoverage)(rep);
        return {
            role: rep.role,
            legacy: c.legacy,
            contracts: c.count,
    ...
    (0, schemas_1.writeArtifact)(runDir, 'contract-coverage.json', { quotas: schemas_1.CONTRACT_QUOTAS, reports: coverageAudit });
    ...
    (0, verifier_1.materializeProbes)(runDir, merged.probes);
    ...
    if (options.probeBaselineGate === false) {
        ...
        baselineGate = (0, verifier_1.aggregateProbeBaselineGates)(baselineGates);
    }
        ...
        baselineGate = await (0, verifier_1.runProbeBaselineGate)(options.workspaceRoot, merged.probes, path.join(runDir, 'baseline-gate', 'all'), options.pythonPath);
    }
    (0, schemas_1.writeArtifact)(runDir, 'probe-baseline-gate.json', baselineGate);
    ...
    if (roleMode === 'probe') {
        ...
        emit({
            type: 'finished',
            ok: true,
            summary: `SciProber 完成：${reports.length} 份勘查报告、隐藏契约 ${merged.contracts.length} 条（structure_map ${merged.chains.length} 条）、物化探针 ${merged.probes.length} 个（probes/）` +
...
${knowledgeBlock}${patcherSkillBlock}`.trim();
    ...
    const finishWithPatch = async (source, patchText, note) => {
        const applied = await applyPatchToWorkspace(options.workspaceRoot, patchText, runDir, source);
        (0, schemas_1.writeArtifact)(runDir, 'chosen-patch.json', { source, note, applied });
        state.stage = 'verify';
        emit({ type: 'stage', stage: 'verify', detail: `最终验证（补丁来源：${source}）` });
        const finalVerify = await (0, verifier_1.verify)(options.workspaceRoot, merged.probes, options.publicCommand, runDir, options.pythonPath);
        (0, schemas_1.writeArtifact)(runDir, 'verify-final.json', finalVerify);
    ...
    const mentorPolishFallback = async (comparison, guidance, round, verdict, progressFile) => {
        let selection = null;
        let selectionSource = '';
        if (verdict) {
            const rule = (0, mentor_1.resolveMentorSelection)(verdict, comparison);
            if (rule.source === 'sci' || rule.source === 'general' || rule.source === 'ds') {
                ...
                if (outcome.available && outcome.patch.trim().startsWith('diff')) {
                    ...
                    selection = {
                        ...
                        weighted: entry ? (0, mentor_1.mentorWeightedScore)(entry.scores) : 0,
        ...
        if (!selection && progressFile && fs.existsSync(progressFile)) {
            try {
                const lines = (0, mentor_1.parseMentorProgressLines)(fs.readFileSync(progressFile, 'utf8'));
                const fromProgress = (0, mentor_1.resolvePolishSelectionFromProgress)(lines, comparison);
                if (fromProgress) {
        ...
        if (!selection) {
            selection = (0, mentor_1.resolvePolishTargetByVerify)(comparison);
        ...
        const polishedRun = await (0, mentor_1.runMentorPolish)(intake, merged.probes, options.publicCommand, comparison, selection, {
        ...
        if (polishedRun.ok) {
            (0, schemas_1.writeArtifact)(runDir, `repair-response-r${round + 1}-mentor-polish.md`, polishedRun.text);
            ...
            if (polishedDiff.trim().startsWith('diff') && (await patchAppliesToWorkspace(options.workspaceRoot, polishedDiff, `r${round + 1}-mentor-polish`))) {
                ...
                (0, schemas_1.writeArtifact)(runDir, `patch-r${round + 1}-mentor-polished.diff`, polishedDiff);
    ...
    for (let round = 0; round <= options.maxRetries; round++) {
        ...
        const runOneDebugger = async (source, copy, framing, model, key, provider) => {
            const resumeThreadId = options.expertResumeThreadIds?.[source];
            if (resumeThreadId) {
                activity.record(`[repair:${source}] 断点续跑：恢复上一轮会话 ${resumeThreadId}（沿用其上轮上下文，非全新会话）`);
                emit({ type: 'activity', text: `[repair:${source}] 断点续跑：恢复会话 ${resumeThreadId}` });
            }
            const agent = await (0, runner_1.runCodexOnce)(buildRepairPrompt(intake, reports, retryFeedback, framing, options.repairTimeoutMs, baselineGate), {
                ...optionsWithActivity,
                sandbox: 'danger-full-access',
                cwd: copy,
            ...
            (0, schemas_1.writeArtifact)(runDir, `repair-response-r${round + 1}-${source}.md`, responseText);
            // 同步镜像到 patcher 角色文件夹（自名子目录，供人工审查；legacy 顶层工件保留）
            const patcherAgent = PATCHER_AGENT_NAMES[source] ?? source;
            (0, schemas_1.writeArtifact)(runDir, `patcher/${patcherAgent}/repair-response-r${round + 1}.md`, responseText);
            ...
            if (capturedDiff) {
                ...
                const verifyResult = await (0, verifier_1.verify)(copy, merged.probes, options.publicCommand, runDir, options.pythonPath);
                (0, schemas_1.writeArtifact)(runDir, `verify-result-r${round + 1}-${source}.json`, verifyResult);
                (0, schemas_1.writeArtifact)(runDir, `patcher/${patcherAgent}/patch-r${round + 1}.diff`, patchText);
                (0, schemas_1.writeArtifact)(runDir, `patcher/${patcherAgent}/verify-result-r${round + 1}.json`, verifyResult);
            ...
            const emptyVerify = await (0, verifier_1.verify)(copy, merged.probes, options.publicCommand, runDir, options.pythonPath);
            (0, schemas_1.writeArtifact)(runDir, `verify-result-r${round + 1}-${source}.json`, emptyVerify);
            (0, schemas_1.writeArtifact)(runDir, `patcher/${patcherAgent}/verify-result-r${round + 1}.json`, emptyVerify);
            return {
                available: false,
                patch: '',
        ...
        if (resumeThisRound) {
            ...
            for (const repairCopy of [sciCopy, generalCopy, dsCopy]) {
                (0, verifier_1.materializeProbes)(repairCopy, merged.probes, 'verify/probes');
            ...
            const repairEnabled = (s) => enabledRepairRoles.includes(s);
            const skippedRepair = (source) => ({
                available: false,
                patch: '',
            ...
            const [sciOutcome, generalOutcome, dsOutcome] = await Promise.all([
                repairEnabled('sci')
                    ? runOneDebugger('sci', sciCopy, sciFraming, options.expertModels?.[sciRole], options.expertKeys?.[sciRole], options.expertProviders?.[sciRole])
                ...
                repairEnabled('ds')
                    ? runOneDebugger('ds', dsCopy, dsFraming, options.expertModels?.['ds'], options.expertKeys?.['ds'], options.expertProviders?.['ds'])
            ...
            const enabledOutcomes = ['sci', 'general', 'ds']
        ...
        (0, schemas_1.writeArtifact)(runDir, `comparison-r${round + 1}.json`, comparison);
        if (roleMode === 'patcher') {
        ...
        const mentor = await (0, mentor_1.runMentor)(intake, merged.probes, options.publicCommand, comparison, {
            codexPath: options.codexPath,
            model: options.mentorModel || options.model,
            sandbox: 'danger-full-access',
            cwd: mentorScratch,
        ...
        if (fs.existsSync(mentorProgressFile)) {
            (0, schemas_1.writeArtifact)(runDir, `mentor-progress-r${round + 1}.jsonl`, fs.readFileSync(mentorProgressFile, 'utf8').trimEnd());
        }
        (0, schemas_1.writeArtifact)(runDir, `mentor-verdict-r${round + 1}.json`, mentor);
        if (mentor.ok && mentor.verdict) {
            ...
            if (verdict.verdict === 'accept') {
                ...
                if (Array.isArray(verdict.evaluation) && verdict.evaluation.length > 0) {
                    (0, schemas_1.writeArtifact)(runDir, `mentor-evaluation-r${round + 1}.md`, (0, mentor_1.renderMentorEvaluationMarkdown)(verdict));
                }
                let sourceLabel;
                let patchToApply;
                let note;
                if (verdict.choice === 'synthesized') {
                    ...
                    const selection = (0, mentor_1.resolveMentorSelection)(verdict, comparison);
                    if (selection.source === 'regenerate') {
                        ...
                        if (regenUsed < maxRegen && roleMode !== 'mentor') {
                            ...
                            retryFeedback = buildDualRetryFeedback([...verdict.semantic_issues, ...selection.filteredOut], verdict.guidance, comparison);
                            (0, schemas_1.writeArtifact)(runDir, 'run-state.json', state);
                ...
                if (sourceLabel === 'synthesized' || sourceLabel === 'mentor-polished') {
                    (0, schemas_1.writeArtifact)(runDir, `patch-r${round + 1}-${sourceLabel}.diff`, patchToApply);
                    emit({
                        type: 'patch',
                        round: round + 1,
                        source: 'mentor',

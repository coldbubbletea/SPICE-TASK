...
exports.MENTOR_RUBRIC_WEIGHTS = void 0;
exports.mentorWeightedScore = mentorWeightedScore;
exports.resolveMentorSelection = resolveMentorSelection;
exports.renderMentorEvaluationMarkdown = renderMentorEvaluationMarkdown;
exports.parseMentorProgressLines = parseMentorProgressLines;
exports.mergeMentorProgress = mergeMentorProgress;
exports.summarizeMentorProgress = summarizeMentorProgress;
exports.buildMentorPrompt = buildMentorPrompt;
exports.resolvePolishSelectionFromProgress = resolvePolishSelectionFromProgress;
exports.resolvePolishTargetByVerify = resolvePolishTargetByVerify;
exports.runMentor = runMentor;
exports.buildMentorPolishPrompt = buildMentorPolishPrompt;
exports.runMentorPolish = runMentorPolish;
...
const CANDIDATE_TO_SOURCE = {
    candidate_1: 'sci',
    candidate_2: 'general',
    candidate_3: 'ds'
};
/** Rubric 权重（Scientific Validity 权重最高，30%）。 */
exports.MENTOR_RUBRIC_WEIGHTS = {
    functional: 0.2,
    numerical: 0.2,
    scientific: 0.3,
    robustness: 0.15,
...
function mentorWeightedScore(scores) {
    return Object.keys(exports.MENTOR_RUBRIC_WEIGHTS)
        .reduce((sum, key) => sum + (Number.isFinite(scores[key]) ? scores[key] : 0) * exports.MENTOR_RUBRIC_WEIGHTS[key], 0);
}
/**
 * Selection = 硬约束 + 加权分：
 ...
 */
function resolveMentorSelection(verdict, comparison) {
    ...
    if (entries.length > 0) {
        ...
        for (const e of entries) {
            const outcome = e.candidate === 'candidate_1' ? comparison.sci
    ...
    const fallback = choice === 'candidate_1' ? 'sci'
...
function renderMentorEvaluationMarkdown(verdict) {
    const lines = ['# SciReviewer rubric 评价报告', ''];
...
const MENTOR_CHOICE_VALUES = ['sci', 'general', 'ds', 'synthesized', 'candidate_1', 'candidate_2', 'candidate_3'];
function isDimensionScores(value) {
    if (!value || typeof value !== 'object') {
        return false;
    }
    const o = value;
    return Object.keys(exports.MENTOR_RUBRIC_WEIGHTS).every((key) => typeof o[key] === 'number' && Number.isFinite(o[key]));
}
function isCandidateEval(value) {
    if (!value || typeof value !== 'object') {
        return false;
    }
    const o = value;
...
function validateMentorVerdict(value) {
    const v = value;
    if (!v ||
    ...
    const choiceOk = v.verdict === 'regenerate'
...
function probeCriteria(probes) {
    return probes
        .map((p) => `- [ ] [参考证据，不计入验收] 探针 ${p.id}（${p.title}）：${p.expect}`)
        .join('\n');
}
/** 解析进度文件（JSONL）为进度行；忽略无法解析的行。 */
function parseMentorProgressLines(text) {
    const out = [];
    for (const raw of text.split('\n')) {
        ...
        try {
            ...
            if (o && typeof o === 'object' && typeof o.candidate === 'string')
                out.push(o);
        }
        catch {
            // 进度文件允许被截断/损坏，忽略坏行
...
function mergeMentorProgress(verdict, lines) {
    ...
    for (const line of lines) {
        ...
        if (line.dimension !== undefined && typeof line.score === 'number' && Number.isFinite(line.score)
            && MENTOR_PROGRESS_DIMENSIONS.includes(line.dimension)) {
    ...
    for (const [candidate, entry] of byCandidate) {
        ...
        evals.push({
            candidate,
            scores: {
                functional: entry.scores.functional,
                numerical: entry.scores.numerical,
                scientific: entry.scores.scientific,
                robustness: entry.scores.robustness,
                intent: entry.scores.intent,
...
/** 一行式进度摘要，供活动日志展示。 */
function summarizeMentorProgress(lines) {
    const order = [];
    const dimScores = new Map();
    const cse = new Map();
    for (const line of lines) {
        if (!order.includes(line.candidate))
            order.push(line.candidate);
        const m = dimScores.get(line.candidate) ?? {};
        if (line.dimension && typeof line.score === 'number')
            m[line.dimension] = line.score;
        dimScores.set(line.candidate, m);
        if (typeof line.cse === 'boolean')
            cse.set(line.candidate, line.cse);
    }
...
function buildMentorPrompt(intake, probes, publicCommand, comparison, context) {
...
${promptClauses_1.MENTOR_RUBRIC_CLAUSE}
...
function resolvePolishSelectionFromProgress(lines, comparison) {
    const byCandidate = new Map();
    for (const line of lines) {
        const entry = byCandidate.get(line.candidate) ?? { scores: {} };
        if (line.dimension !== undefined &&
    ...
    const dimKeys = Object.keys(exports.MENTOR_RUBRIC_WEIGHTS);
    const pairs = [
    ...
    const ranked = pairs
        .filter((pair) => pair.outcome.available &&
        pair.outcome.patch.trim().startsWith('diff') &&
        pair.entry !== undefined &&
        pair.entry.cse !== true &&
        dimKeys.every((dim) => typeof pair.entry.scores[dim] === 'number'))
        .map((pair) => ({ ...pair, weighted: mentorWeightedScore(pair.entry.scores) }))
        .sort((a, b) => b.weighted - a.weighted);
    if (ranked.length === 0)
        return null;
    const winner = ranked[0];
    const loserNote = ranked.length > 1
...
function resolvePolishTargetByVerify(comparison) {
    const zeroScores = {
        functional: 0,
        numerical: 0,
        scientific: 0,
...
async function runMentor(intake, probes, publicCommand, comparison, options) {
    const runOptions = {
    ...
    let result = await (0, runner_1.runCodexOnce)(prompt, runOptions);
    let verdict;
    if (result.ok) {
        verdict = (0, schemas_1.extractJson)(result.text);
        if (!validateMentorVerdict(verdict) && result.threadId) {
            result = await (0, runner_1.runCodexOnce)('你的输出不符合 schema。请只输出修正后的 JSON 对象（verdict/choice/patch/patch_file/evaluation/ranking/semantic_issues/guidance），不要输出其他内容。打磨补丁过长时可写入当前目录 polished-patch.diff 并只填 patch_file。', { ...runOptions, resumeThreadId: result.threadId });
            verdict = (0, schemas_1.extractJson)(result.text);
        }
    }
    if (result.ok && validateMentorVerdict(verdict)) {
        ...
        if (verdict.patch_file && !(verdict.patch ?? '').trim().startsWith('diff')) {
            const patchFilePath = path_1.default.resolve(options.cwd, verdict.patch_file.trim());
            if (fs_1.default.existsSync(patchFilePath)) {
                const fromFile = fs_1.default.readFileSync(patchFilePath, 'utf8').trim();
        ...
        if (options.progressFile && fs_1.default.existsSync(options.progressFile)) {
            const lines = parseMentorProgressLines(fs_1.default.readFileSync(options.progressFile, 'utf8'));
...
function buildMentorPolishPrompt(intake, probes, publicCommand, comparison, selection, options) {
    const SOURCE_TO_LABEL = {
        sci: 'candidate_1',
        general: 'candidate_2',
        ds: 'candidate_3'
    };
    const others = ['sci', 'general', 'ds']
    ...
    const othersSections = others
        ...
        .join('\n\n');
    const scoreLines = Object.keys(exports.MENTOR_RUBRIC_WEIGHTS)
        .map((dim) => `- ${String(dim).padEnd(14)} ${String(selection.scores[dim]).padStart(4)} （权重 ${(exports.MENTOR_RUBRIC_WEIGHTS[dim] * 100).toFixed(0)}%）`)
        .join('\n');
...
function runMentorPolish(intake, probes, publicCommand, comparison, selection, options) {
    return (0, runner_1.runCodexOnce)(buildMentorPolishPrompt(intake, probes, publicCommand, comparison, selection, options), { ...options, sandbox: 'danger-full-access' });
}
